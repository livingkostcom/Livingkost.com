...
<div>
                        ...
                        <tr class="hover:bg-blue-50 transition duration-200">
                            ...
                            <td class="px-6 py-4 text-sm space-x-2 flex">
                                ...
                                @if ($invoice->verified_at)
                                    <button disabled
                                        class="inline-flex items-center px-3 py-1 bg-gray-100 text-gray-400 rounded cursor-not-allowed font-medium"
                                        title="Invoice sudah diverifikasi, tidak dapat diubah">
                                        Edit
                                    </button>
                                @else
                                ...
                                @if ($invoice->verified_at)
                                    <button disabled
                                        class="inline-flex items-center px-3 py-1 bg-gray-100 text-gray-400 rounded cursor-not-allowed font-medium"
                                        title="Invoice sudah diverifikasi, tidak dapat dihapus">
                                        Hapus
                                    </button>
                                @else
                                ...
                                @if ($invoice->status === 'unpaid')
                                    <button wire:click="sendReminder({{ $invoice->id }})"
                                        ...
                                        class="inline-flex items-center px-3 py-1 bg-yellow-100 text-yellow-700 rounded hover:bg-yellow-200 transition font-medium disabled:opacity-50"
                                        title="Kirim pengingat pembayaran">
                                        <span wire:loading.remove
                                            wire:target="sendReminder({{ $invoice->id }})">Ingatkan</span>
    ...
    @if ($showGenerateModal)
        <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div class="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[85vh] overflow-hidden animate-fade-in">
                <!-- Header -->
                <div
