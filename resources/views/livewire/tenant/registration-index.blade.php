<div class="max-w-6xl mx-auto">
    ...
    <!-- Welcome-message sender (shown right after an approval) -->
    @if ($welcomeMessage)
        <div x-data="{
                phone: @js($welcomePhone),
                msg: @js($welcomeMessage),
                showPreview: false,
                copied: false,
                get link() { return 'https://wa.me/' + this.phone + '?text=' + encodeURIComponent(this.msg); }
             }"
            class="mb-5 bg-green-50 border border-green-200 rounded-2xl p-5">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <p class="text-sm font-bold text-green-800">Kirim pesan selamat datang ke {{ $welcomeName }}</p>
                    <p class="text-xs text-green-700 mt-0.5">Klik tombol untuk membuka WhatsApp dengan pesan sudah terisi @if ($welcomePhone) ke nomor <span class="font-semibold">+{{ $welcomePhone }}</span> @endif.</p>
                </div>
                <div class="flex items-center gap-2 shrink-0">
                    <button type="button" @click="showPreview = !showPreview"
                        class="px-3 py-2 text-xs font-semibold text-green-700 hover:text-green-900">
                        <span x-show="!showPreview">Lihat pesan</span>
                        <span x-show="showPreview" x-cloak>Sembunyikan</span>
                    </button>
                    <button type="button"
                        @click="navigator.clipboard.writeText(msg).then(() => { copied = true; setTimeout(() => copied = false, 2000); })"
                        class="px-3 py-2 bg-white border border-green-300 text-green-700 rounded-lg text-xs font-semibold hover:bg-green-100 transition whitespace-nowrap">
                        <span x-show="!copied">Salin teks</span>
                        <span x-show="copied" x-cloak>Tersalin ✓</span>
                    </button>
                    <a :href="link" target="_blank"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-semibold hover:bg-green-700 transition whitespace-nowrap">
            ...
            <div x-show="showPreview" x-cloak class="mt-3 bg-white border border-green-100 rounded-xl p-4">
                <pre class="whitespace-pre-wrap break-words text-xs text-gray-700 font-sans" x-text="msg"></pre>
            </div>
        </div>
    @endif
        ...
        @forelse ($registrations as $reg)
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    ...
                    @elseif ($reg->status === 'approved' && isset($welcomeLinks[$reg->id]))
                        <div class="shrink-0"
                            x-data="{ phone: @js($welcomeLinks[$reg->id]['phone']), msg: @js($welcomeLinks[$reg->id]['message']) }">
                            <a :href="'https://wa.me/' + phone + '?text=' + encodeURIComponent(msg)" target="_blank"
                                class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-semibold hover:bg-green-700 transition whitespace-nowrap">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51l-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.71.306 1.263.489 1.694.625.712.227 1.36.195 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                                Kirim WhatsApp
                            </a>
                        </div>
