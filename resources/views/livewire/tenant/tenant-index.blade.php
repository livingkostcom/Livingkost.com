<div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1
                    ...
                    Penyewa</h1>
                <p class="mt-2 text-gray-600">Kelola semua data penyewa Anda</p>
            </div>
    ...
    <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="relative group">
            ...
            <input wire:model.live="search" type="text" placeholder="Cari NIK, nama, atau email..."
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($tenants as $tenant)
                        <tr class="hover:bg-orange-50 transition duration-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <p class="text-gray-900 font-mono text-sm">{{ $tenant->nik }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <p class="text-gray-600 text-sm">{{ $tenant->email }}</p>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    ...
                                    @elseif ($tenant->status === 'inactive')
                                        <span class="w-2 h-2 rounded-full bg-yellow-500 mr-2"></span>Tidak Aktif
                                    @else
                                        <span class="w-2 h-2 rounded-full bg-red-500 mr-2"></span>Keluar
                                    @endif
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                ...
                                <p class="text-gray-500">Tidak ada penyewa ditemukan</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden animate-fade-in">
                ...
                <div class="bg-gradient-to-r from-orange-600 to-orange-600 px-6 py-4 flex items-center justify-between">
                    <h2 class="text-lg font-bold text-white">
                        {{ $editingId ? 'Edit Penyewa' : 'Tambah Penyewa Baru' }}
                    </h2>
                ...
                <!-- Form -->
                <div class="p-6">
                    <livewire:tenant.tenant-form :tenantId="$editingId" wire:key="form-{{ $editingId }}" />
                </div>
            </div>
        </div>
    @endif
    ...
    @if ($showDeleteModal && $deletingTenant)
        <div class="fixed inset-0 bg-black/20 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-fade-in">
                ...
                <!-- Content -->
                <div class="p-6">
                    <p class="text-gray-700 mb-3 font-medium">Apakah Anda yakin ingin menghapus penyewa berikut?</p>
                    <div
                        class="bg-gradient-to-br from-red-50 to-pink-50 p-4 rounded-xl mb-6 border-l-4 border-red-500">
                        <p class="text-sm text-gray-600">Nama</p>
                        <p class="text-lg font-bold text-gray-900">{{ $deletingTenant->display_name }}</p>
                        <p class="text-sm text-gray-600 mt-2">NIK</p>
                        <p class="text-gray-900 font-mono">{{ $deletingTenant->nik }}</p>
                        <p class="text-sm text-gray-600 mt-2">Email</p>
                        <p class="text-gray-900">{{ $deletingTenant->email }}</p>
                    </div>
                    <p class="text-sm text-red-600 font-semibold mb-6">
                        Tindakan ini tidak dapat dibatalkan. Data penyewa akan dihapus secara permanen.
                    </p>
                </div>
