<div>
    ...
    @if ($isSuperAdmin && $platformSummary)
        <!-- Pendapatan Bersih Platform (super-admin) -->
        <div class="bg-gradient-to-br from-emerald-50 to-white rounded-2xl shadow-md border border-emerald-200 p-6 mb-6">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
                <div>
                    <p class="text-sm text-gray-500">Pendapatan Bersih Platform (periode dipilih)</p>
                    <p class="text-3xl sm:text-4xl font-bold {{ $platformSummary['net_income'] >= 0 ? 'text-emerald-600' : 'text-red-600' }}">
                        Rp {{ number_format($platformSummary['net_income'], 0, ',', '.') }}
                    </p>
                ...
                <div class="grid grid-cols-3 gap-4 sm:gap-6 text-sm">
                    <div>
                        ...
                        <p class="font-bold text-emerald-600 whitespace-nowrap">Rp {{ number_format($platformSummary['fee_income'], 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500">Pendapatan Lain</p>
                        <p class="font-bold text-emerald-600 whitespace-nowrap">+ Rp {{ number_format($platformSummary['other_income'], 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500">Pengeluaran</p>
                        <p class="font-bold text-red-500 whitespace-nowrap">− Rp {{ number_format($platformSummary['company_expense'], 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
        </div>
    @else
        <!-- Net Income Banner (owner) -->
